package com.flashcms.layout{
	 ...
	 * (rect).
	 * <br/><br/>
	 * Using the LayoutManager class, Layout instances can be automatically
	 ...
	 * LayoutManager, updates can be forced using the draw() method and
	 * can be propagated through child layouts if a hierarchy has been
	 * defined by setting the parent property for layouts.
	 * <br/><br/>
	 * If LayoutManager is not being used, you would need to update your layout
	 * using the draw() method (this is sometimes still needed when the 
	 ...
	 * a target to the layout's rect when useDefaultChangeHandler is true.
	 ...
	 * with that layout. Child display objects of that target can have layouts
	 * which will be contained within the target's layout. An optional children
	 * property can be set to a Layout or LayoutConstraint instance to define a
	 * sublayout for constraining child layouts.
	 *
	...
	public class Layout extends LayoutConstraint {
		 ...
		 * withinthe current layout.  By default, layouts within
