	...
	*/
	...
	//--------------------------------------	
		...
		public function set position(value:Number):void
		{
			if(_mediaAvailable)
			{	
				//will be used to set the position of the clip based on the position of the scrubber
				if(!_playing)
				{
					_position = value;		
				}
		...
		public function set volume(value:Number):void
		{
			...
			{		
				_channel.soundTransform = _soundTransform;
				dispatchEvent(new MediaEvent(MediaEvent.VOLUME_CHANGE, false, false, Number(_channel.position.toFixed(2)), _clip.length, Number(_channel.soundTransform.volume), _mute));
			}
			else
			{
	...
	//--------------------------------------	
		...
		public function loadMedia(urlValue:String, autoStart:Boolean = true):void
		{
			...
			dispatchEvent(new MediaEvent(MediaEvent.MEDIA_POSITION, false, false, _position, _clip.length, _channel.soundTransform.volume, _mute));
			dispatchEvent(new MediaEvent(MediaEvent.MEDIA_PLAY_PAUSE, false, false, _position, _clip.length, _channel.soundTransform.volume, _mute));
			dispatchEvent(new MediaEvent(MediaEvent.VOLUME_CHANGE, false, false, _position, _clip.length, _channel.soundTransform.volume, _mute));
			if(!autoStart)
			{
				pause();
				_position = 0;
		...
		public function pause():void
		{
			...
			{
				_playing = false;
				dispatchEvent(new MediaEvent(MediaEvent.MEDIA_PLAY_PAUSE, false, false, Number(_channel.position.toFixed(2)), _clip.length, Number(_channel.soundTransform.volume), _mute));
				stop();
				_positionTimer.stop();
			}
		}
	...
	//--------------------------------------		
		...
		protected function positionUpdateHandler(event:TimerEvent):void
		{
			...
			dispatchEvent(new MediaEvent(MediaEvent.MEDIA_POSITION, false, false, _playing?Number(_channel.position.toFixed(2)):_position, _clip.length, Number(_channel.soundTransform.volume), _mute));
		}
		...
		protected function soundCompleteHandler(event:Event):void
		{
			...
			dispatchEvent(new MediaEvent(MediaEvent.MEDIA_POSITION, false, false, _playing?Number(_channel.position.toFixed(2)):_position, _clip.length, _soundTransform.volume, _mute));
			dispatchEvent(new MediaEvent(MediaEvent.MEDIA_ENDED, false, false, _playing?Number(_channel.position.toFixed(2)):_position, _clip.length, _soundTransform.volume, _mute));
		}
