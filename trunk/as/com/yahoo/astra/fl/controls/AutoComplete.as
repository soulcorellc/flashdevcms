...
public class AutoComplete extends TextInput
{
    ...
    /**
     *  @private
     *  A flag to track whether the _dropdown tweened in or out. 
     */
    //private var tweenIn:Boolean = false;
	...
	public function set dataProvider(value:DataProvider):void
    {
		...
		//if(!usingLocalHistory) filteredData = value as Array;
    }
    //----------------------------------
    //  selectedIndex
    //----------------------------------
    ...
    protected function set dropdownWidth(value:Number):void
    {
        ...
        if (_dropdown)
            _dropdown.setSize(value, _dropdown.height);
    ...
    {
        return Math.max(1, Math.min(dataProvider.length, _rowCount));
    }
    /**
     *  @private
     */
	...
	private function updateDataProvider(allowOpen:Boolean= true):void
	{
		...
		dropdown.dataProvider = new DataProvider(filteredData);
	    ...
	    //In case there are no suggestions, check there is something in the localHistory
  	    ...
  	    if(allowOpen) 
		{
			(dropdown.dataProvider.length > 0) ? open(): close();
		
		}
			
  	    dropdown.height = Math.min((filteredData.length * dropdown.rowHeight),100);
		
		_dataProvider = dropdown.dataProvider;
  	}
    ...
    protected function displayDropdown(show:Boolean, trigger:Event = null):void
    {
        ...
        if (show)
        {
            ...
            // at the top. But if the space there is also less than false
            // display it below.
            if (point.y + _dropdown.height > stage.height &&
                point.y > _dropdown.height)
            {
                // Dropdown will go below the bottom of the stage
                // and be clipped. Instead, have it grow up.
                point.y -= (height + _dropdown.height + getStyleValue("focusRectPadding")+ getStyle("borderThickness"));
        ...
        inTween = true;
         ...
         if(show)
         {
         	...
         	if(_dropdown.dataProvider.length < 1)  
            {
            	
            	dropdown.dataProvider = unfilteredDataProvider.clone();
            }
         	_dropdown.alpha = 1;
         ...
         {
         	//TODO
         	//_dropdown.alpha = 1//getStyle("alpha");
         	
         	duration =  Math.max(1, getStyle("closeDuration") as Number);
         	//tween = new Tween(dropdown,'alpha',Regular.easeOut,1,0, 2,true);
        ...
        else
        {
                ...
                event.keyCode == Keyboard.PAGE_DOWN)
            {
            	...
            	if(_dropdown.visible)
            	{
	                ...
	                if(_dropdown)
	                {
	                	if(_dropdown.selectedIndex == dataProvider.length - 1 &&  event.keyCode == Keyboard.DOWN)
						{
							if(loopSelection)
							{
								selectedIndex = 0
